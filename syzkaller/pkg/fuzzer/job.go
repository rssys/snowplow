...
import (
	...
	"github.com/google/syzkaller/pkg/mlargmutator"
...
func (job *triageJob) handleCall(call int, info *triageCall) {
	...
	// add the input to the corpus before doing the smash
	...
	exists := job.fuzzer.Config.Corpus.Save(input)
	job.fuzzer.Logf(2, "added new input for %v to the corpus: %s (exists=%v)", callName, p, exists)
...
func (job *smashJob) run(fuzzer *Fuzzer) {
	...
	if job.isNew {
		go job.tryMLSmash(fuzzer)
	}
	const iters = 25
	rnd := fuzzer.rand()
	for i := 0; i < iters; i++ {
		p := job.p.Clone()
		if job.isNew && fuzzer.Config.UseML {
			p.MutateWithLessArgMutation(rnd, prog.RecommendedCalls,
				fuzzer.ChoiceTable(),
				fuzzer.Config.NoMutateCalls,
				fuzzer.Config.Corpus.Programs())
		} else {
...
func (job *smashJob) tryMLSmash(fuzzer *Fuzzer) {
	var progCover covergraph.ProgCover
	// profile the coverage of this program
	p := job.p.Clone()
	result := fuzzer.execute(job.exec, &queue.Request{
		Prog:     p,
	...
	coverSize := 0
	for _, callInfo := range result.Info.Calls {
		callCover := &covergraph.CallCover{Trace: callInfo.Cover}
		progCover.CallList = append(progCover.CallList, *callCover)
		coverSize += len(callCover.Trace)
	}
	// if we can collect coverage info, run it through ML
	if coverSize == 0 {
		return
	}
	// submit the job to the inference workers
	req := &mlargmutator.Request{
		P:         job.p.Clone(),
		ProgCover: &progCover,
	}
	fuzzer.mlSmashChan <- req
}

type mlSmashJob struct {
	exec        queue.Executor
	p           *prog.Prog
	suggestions map[prog.ArgLocator]bool
}

func (f *Fuzzer) handleMLRequest() {
	for {
		req := <-f.mlSmashChan
		if f.Config.UseML == false {
			continue
		}
		suggestions := f.mlargmutator.PredictProg(req)
		if len(suggestions) == 0 {
			continue
		}
		f.startJob(f.statJobsMLSmash, &mlSmashJob{
			exec:        f.smashQueue,
			p:           req.P.Clone(),
...
func (job *mlSmashJob) run(fuzzer *Fuzzer) {
	fuzzer.Logf(2, "mlSmashing the program %s:", job.p)
	rnd2nd := fuzzer.rand2nd()
	repeatCnt := len(job.suggestions) * fuzzer.Config.MLSmashExecCnt
	for repeat := 0; repeat < repeatCnt; repeat++ {
		p := job.p.Clone()
		p.MutateSpecificArgs(rnd2nd, prog.RecommendedCalls,
			fuzzer.ChoiceTable(),
			fuzzer.Config.NoMutateCalls,
			fuzzer.Config.Corpus.Programs(), job.suggestions)
		p.MutateWithLessArgMutation(rnd2nd, prog.RecommendedCalls,
			fuzzer.ChoiceTable(),
			fuzzer.Config.NoMutateCalls,
			fuzzer.Config.Corpus.Programs())
		result := fuzzer.execute(job.exec,
			&queue.Request{
				Prog:     p,
				ExecOpts: setFlags(flatrpc.ExecFlagCollectSignal),
				Stat:     fuzzer.statExecMLSmash,
			})
		if result.Stop() {
			return
		}

...
type Fuzzer struct {
	...
	mlargmutator *mlargmutator.MLArgMutator
	mlSmashChan  chan *mlargmutator.Request
	execQueues
}

func NewFuzzer(ctx context.Context, cfg *Config, rndSeed int64,
	...
	rnd2nd := rand.New(rand.NewSource(rndSeed))
	f := &Fuzzer{
		Stats:  newStats(),
		Config: cfg,
		Cover:  newCover(),
	...
	// TODO: sishuai: livenecess check on the model
	config := &mlargmutator.MLArgMutatorConfig{
		ChildBlockDict:  path.Join(cfg.KernelAnalysisDir, "block-calling-dict.result"),
		BlockASMDict:    path.Join(cfg.KernelAnalysisDir, "block-asm-dict"),
		BlockFuncDict:   path.Join(cfg.KernelAnalysisDir, "block-func-dict"),
		ASMTokenDict:    path.Join(cfg.KernelAnalysisDir, "asm-token-dict"),
		ShortcutEdgeHop: 8,
		ServerAddr:      cfg.MLServerAddr,
		ServerKey:       "",
	}
	mlmutator := mlargmutator.InitMLArgMutator(config)
	f.mlargmutator = mlmutator
	f.mlSmashChan = make(chan *mlargmutator.Request, 10000)
	// NOTE: we can achieve the best balance between throughput and latency with
	// 40 concurrent workers
	for i := 0; i < 40; i++ {
		go f.handleMLRequest()
	}
	return f
}
...
func (fuzzer *Fuzzer) rand2nd() *rand.Rand {
	fuzzer.mu.Lock()
	defer fuzzer.mu.Unlock()
	return rand.New(rand.NewSource(fuzzer.rnd2nd.Int63()))
}

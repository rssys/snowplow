...
func (mgr *Manager) initBench() {
	...
	go func() {
		for {
			// sishuai: let the bench log start from the same uptime
			if time.Duration(mgr.statUptime.Val())*time.Second == 60*time.Second {
				break
			}
		}
		mgr.writeBench()
...
func (mgr *Manager) MachineChecked(features flatrpc.Feature, enabledSyscalls map[*prog.Syscall]bool) queue.Source {
	...
	if mgr.mode == ModeFuzzing {
		...
		fuzzerObj := fuzzer.NewFuzzer(context.Background(), &fuzzer.Config{
			...
			KernelAnalysisDir: mgr.cfg.KernelAnalysisDir,
			UseML:             mgr.cfg.UseML,
			MLSmashExecCnt:    mgr.cfg.MLSmashExecCnt,
			MLServerAddr:      mgr.cfg.MLServerAddr,
		}, mgr.cfg.InitSeed, mgr.target)
		...
		ctx := &corpusRunner{
			candidates: corpus,
			// rnd:        rand.New(rand.NewSource(time.Now().UnixNano())),
			rnd: rand.New(rand.NewSource(mgr.cfg.InitSeed)),
		}

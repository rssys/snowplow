...
func (p *Prog) MutateWithLessArgMutation(rs rand.Source, ncalls int, ct *ChoiceTable, noMutate map[int]bool, corpus []*Prog) {
	p.MutateWithOpts(rs, ncalls, ct, noMutate, corpus, LessArgMutateOpts)
}
...
var LessArgMutateOpts = MutateOpts{
	ExpectedIterations: 5,
	MutateArgCount:     2,
...
func updateArgLocList(mutatedArgLoc ArgLocator, numNewCalls int, argLocList []ArgLocator) []ArgLocator {
	var newArgLocList []ArgLocator
	for _, argLoc := range argLocList {
		// no further mutation on the mutated call's args
		// this is because the mutation of one arg might change
		// the arg list a lot so the old idx may not hold anymore
		if argLoc.CallIdx == mutatedArgLoc.CallIdx {
			continue
		}
		// only update the calls after and including the mutatedArgLoc
		// calls before the mutatedArgLoc are not affected
		// as the new calls must be inserted after them
		if argLoc.CallIdx > mutatedArgLoc.CallIdx {
			argLoc.CallIdx += numNewCalls
		}
		newArgLocList = append(newArgLocList, argLoc)
	}
	return newArgLocList
}

func (p *Prog) MutateSpecificArgs(rs rand.Source, ncalls int, ct *ChoiceTable, noMutate map[int]bool, corpus []*Prog, suggestions map[ArgLocator]bool) {
	var argLocList []ArgLocator
	const earlyReturn=20
	...
	for argLoc := range suggestions {
		argLocList = append(argLocList, argLoc)
	}

	failureCnt := 0
	for stop, ok := false, false; !stop; stop = ok && r.oneOf(3) {
		argLoc := argLocList[r.Intn(len(argLocList))]
		ok, numInsertedCalls := ctx.mutateThisArg(argLoc)
		if !ok {
			failureCnt += 1
			if failureCnt == earlyReturn {
				// TODO: a better way to log this
				break
			}
			continue
		}
		argLocList = updateArgLocList(argLoc, numInsertedCalls, argLocList)
		if len(argLocList) == 0 {
			break
		}
	}
	p.sanitizeFix()
	p.debugValidate()
	// Comments: deleted the call number check by Syzkaller
}

func (ctx *mutator) mutateThisArg(argLoc ArgLocator) (bool, int) {
	const earlyReturn=10
	...
	ForeachArg(c, ma.collectArgNonstop)
	if len(ma.args) == 0 {
		panic("no args found for the selected call")
	}
	s := analyze(ctx.ct, ctx.corpus, p, c)
	arg, argCtx := ma.locateMutateArg(argLoc)
	failureCnt := 0
	for stop, ok := false, false; !stop; stop = ok {
		...
		p.insertBefore(c, calls)
		numInsertedCalls = len(calls)
		// no need to respect the length limit for this type of mutation
		/*
		idx += len(calls)
		for len(p.Calls) > ctx.ncalls {
			idx--
	...
	return true, numInsertedCalls
}

func (ma *mutationArgs) locateMutateArg(argLoc ArgLocator) (Arg, ArgCtx) {
	argIdx := argLoc.ArgIdx
	if argIdx < 0 || argIdx >= len(ma.args) {
		panic("argIdx out of bound")
	}
	return ma.args[argIdx].arg, ma.args[argIdx].ctx
}

func IsArgMutatable(p *Prog, argLoc ArgLocator) bool {
	ma := &mutationArgs{target: p.Target}
	c := p.Calls[argLoc.CallIdx]
	ForeachArg(c, ma.collectArgNonstop)
	if len(ma.args) == 0 {
		panic("no args found for the selected call")
	}
	if argLoc.ArgIdx < 0 || argLoc.ArgIdx >= len(ma.args) {
		err := fmt.Errorf("callIdx=%v, argIdx=%v | numCall=%v, numArg=%v", argLoc.CallIdx, argLoc.ArgIdx, len(p.Calls), len(ma.args))
		panic(err)
	}
	return !ma.args[argLoc.ArgIdx].cannotMutate
}

func CountCallArg(p *Prog, callIdx int) int {
	c := p.Calls[callIdx]
	ma := &mutationArgs{target: p.Target}
	ForeachArg(c, ma.collectArgNonstop)
	return len(ma.args)
...
func (ma *mutationArgs) collectArg(arg Arg, ctx *ArgCtx) {
	...
	ma.prioSum += prio
	ma.args = append(ma.args, mutationArg{arg, *ctx, ma.prioSum, false})
}

// a arg collect func to collect all args
func (ma *mutationArgs) collectArgNonstop(arg Arg, ctx *ArgCtx) {
	ignoreSpecial := ma.ignoreSpecial
	ma.ignoreSpecial = false

	typ := arg.Type()
	prio, _ := typ.getMutationPrio(ma.target, arg, ignoreSpecial)
	// we never stop in order to collect all args
	ctx.Stop = false
	...
	ma.prioSum += prio
	ma.args = append(ma.args, mutationArg{arg, *ctx, ma.prioSum, cannotMutate})
}
